...
const getProfessorDashboardStats = asyncHandler(async (req, res) => {
  const professorUserId = req.user._id;

  const professor = await Professor.findOne({ userId: professorUserId });
  const courseCount = professor?.assignedCourses?.length || 0;

  const assignmentsCount = await Assignment.countDocuments({ professorId: req.user._id });
